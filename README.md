...
На вкладке проекта `Готовность` можно включить режим готового проекта. После предупреждения становится доступна активная curl-like проверка доступности: приложение отправляет HTTP/HTTPS запросы к доменам и поддоменам проекта, сохраняет статусы, время ответа и итог `OK/FAIL`. Запускайте этот режим только для ресурсов, на которые есть разрешение.
